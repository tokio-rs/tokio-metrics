...
macro_rules! metric_refs {
    (
        ...
        struct $struct_name {
            ...
            $(
                #[cfg(tokio_unstable)]
                $unstable_name: kind_to_type!($unstable_kind),
            )*
        }
        ...
        impl $struct_name {
            fn capture(transform_fn: &mut dyn FnMut(&'static str) -> metrics::Key) -> Self {
                Self {
                    ...
                    $(
                        #[cfg(tokio_unstable)]
                        $unstable_name: capture_metric_ref!(transform_fn, $unstable_name: $unstable_kind $unstable_opts),
                    )*
                }
            }
            ...
            fn describe(transform_fn: &mut dyn FnMut(&'static str) -> metrics::Key) {
                ...
                $(
                    #[cfg(tokio_unstable)]
                    describe_metric_ref!(transform_fn, $unstable_doc, $unstable_name: $unstable_kind<$unstable_unit> $unstable_opts);
                )*
            }
        }
